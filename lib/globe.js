...
function onMouseClick(event) {
    ...
    const passportCountries = passportIdx();
    ...
    raycaster.setFromCamera(mouse, camera); {
        ...
        if (intersects.length > 0) {
            ...
            let clickedCountry = clickedObj.parent.userData["userText"].toString();
              for (let i = 0; i < passportCountries.length; i++) {
                let countryObj = passportCountries[i];
                if (countryObj[clickedCountry] !== undefined) {
                   return ( $('#country').text(countryObj[clickedCountry].toString()));
                }
            }
