...
import passportIdx from './routes';
import countryCoordinates from './coords';
...
var someColors = [0x000000];
    // 0x808080, 0xa0a0a0, 0x929292, 0x858585, 0xa9a9a9
...
function onMouseClick(event) {
    ...
    const passportData = passportIdx();
    const latAndLongData = countryCoordinates();
    ...
    raycaster.setFromCamera(mouse, camera); {
        ...
        let setGroupColor = function (group, colorHex) {
            ...
            for (let i = 0; i < group.children.length; i++) {
                if (!group.children[i].userData.color) {
                    group.children[i].userData.color = clickedObj.parent.children[i].material.color.clone();
                    group.children[i].material.color.set(colorHex);
                    group.children[i].material.needsUpdate = true;
                }
            }
        ...
        if (intersects.length > 0) {
            ...
            // set colors for hovered group
            // debugger
            setGroupColor(clickedObj.parent, 0x80FF80);
            // debugger
            $('#country').css('display', 'block');
            // $('#country').text("Nope, that's " + clickedObj.parent.userData["userText"].toString());
            let clickedCountry = clickedObj.parent.userData["userText"];
            let passportIdxArr = passportData[clickedCountry];
            let startCoords = latAndLongData[clickedCountry];
            // console.log(startCoords);

            // console.log(latAndLongData[passportIdxArr[0]][0])
            let endCoords = [];
            for (let i = 0; i < passportIdxArr.length; i++) {
                if (latAndLongData[passportIdxArr[i]] === undefined) {
                    continue;
                }
                    let endLat = latAndLongData[passportIdxArr[i]][0];
                    let endLong = latAndLongData[passportIdxArr[i]][1];
                    endCoords.push([endLat, endLong]);

            };

            let allCoords = []
            for (let i = 0; i < endCoords.length; i++) {
                startCoords.push(endCoords[i])
            }
            ...
            // console.log(startCoords.concat(endCoords));
            // console.log(clickedCountry);
            // console.log(passportData);
            
            $('#country').text(clickedCountry);
        //     if ((clickedObj.parent.userData["userText"].toString()) === "Sri Lanka") {
        //         $('#country').text("yay. that's all. and the world keeps spinning...");
        //         $('#country').css('color', 'green')
        //         $('#gameover').css('display', 'block');
        //         $('#gameover').text("LOL you got it!");
